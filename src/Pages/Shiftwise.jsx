...
export default function Shiftwise() {
  ...
  const getDefaultShift = () => {
    ...
    // If current time is between 9 AM and 6 PM, set Shift 1, otherwise Shift 2
    return (hour >= 9 && hour < 18) ? "1" : "2";
  };
  ...
  const fetchData = async () => {
    try {
        ...
        const response = await axios.get('http://192.168.137.161:3000/shiftwise' , {
        params: {
          id: machineId,
  ...
  useEffect(() => {
    // Set the current date as default when the component mounts
    setDate(dayjs());
  }, []);
